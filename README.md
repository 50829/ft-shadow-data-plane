...
Binance USD-M 正式数据采集与重建流水线。v0.2.0 从 generation 1 直接采集 60 个合约：
...
Vultr 负责采集、每日市场观测、排名和增量换币。107 只负责每分钟短时拉取、持久化校验、
ACK 和 Slurm 处理，不参与选币。成员不变的 UTC 日切不会停止数据源；替换一个币只在线更新
这个币涉及的订阅和 OI 任务，其余 59 个币保持在线。

正式规则、边界语义和性能标准见 [实施合同](docs/implementation-plan.md)。部署入口：
